    ...
    >
      ...
      {data && (
        <div className="flex flex-col gap-3">
          ...
          {/* Pays / WHT — dividendes */}
          <div
            className="flex flex-wrap items-end gap-2 rounded-lg border border-[var(--border)] bg-[var(--muted)]/20 px-2.5 py-2"
          ...
          >
            <div className="min-w-[5rem]">
              ...
              <input
                className="input mt-0.5 py-1 font-mono uppercase"
                maxLength={2}
                placeholder="US"
            ...
            <button
              type="button"
              className="rounded-md bg-slate-800 px-2.5 py-1.5 text-[11px] font-medium text-white dark:bg-slate-200 dark:text-slate-900"
              onClick={async () => {
                const cc = (
                  document.getElementById("asset-country-code") as HTMLInputElement
                )?.value;
                const wht = (
                  document.getElementById("asset-wht-rate") as HTMLInputElement
                )?.value;
                try {
                  ...
                  // soft refresh via reload
                  window.dispatchEvent(new Event("focus"));
                } catch (e) {
                  alert(e instanceof Error ? e.message : "Erreur sauvegarde pays/WHT");
                }
              }}
            >
              Enregistrer pays / WHT
            </button>
            <p className="w-full text-[10px] text-slate-400">
              Pays ISO (US, DE, FR…) pour le prélèvement à la source. Override
              optionnel (0–1 ou %). PEA : pas de PFU dans la perf ; WHT étranger
              reste.
            </p>
          </div>
          ...
          <div>
            <div className="mb-1.5 flex flex-wrap items-center justify-between gap-2">
              <h4 className="text-sm font-semibold">Historique des transactions</h4>
              <span className="text-[11px] text-slate-500 tabular-nums">
                {filteredTxs.length}/{txs.length}
              </span>
            </div>
            <TxTypeFilters
              value={typeFilter}
...
function FxPnlPanel({
  ...
  return (
    <div
      className="rounded-lg border border-[var(--border)] bg-slate-50/60 px-2.5 py-2 dark:bg-slate-900/40"
      data-testid="fx-pnl-panel"
    >
      <div className="mb-1 text-[10px] font-semibold uppercase tracking-wide text-slate-500">
        P&amp;L latent — prix vs change
      </div>
      <div className="grid grid-cols-3 gap-2 text-center sm:text-left">
        ...
        <div>
          <div className="text-[9px] text-slate-400">Effet prix</div>
          <div
            className={cn(
        ...
        <div>
          <div className="text-[9px] text-slate-400">Effet change</div>
          <div
            className={cn(
              "text-sm font-semibold tabular-nums",
              d.isEur ? "text-slate-400" : getChangeColor(d.fxPnlEur)
            )}
          >
            {d.isEur ? "—" : formatCurrency(d.fxPnlEur, "EUR")}
          </div>
        </div>
      </div>
      <p className="mt-1 text-[9px] leading-snug text-slate-400">{d.note}</p>
    </div>
  );
}
