...
| `ZERION_API_KEY` | Wallets EVM multi-chain | Non |
| `OPENSEA_API_KEY` | NFT OpenSea API v2 (`x-api-key`) | Non (auto free-tier si `OPENSEA_AUTO_KEY`) |
| `OPENSEA_AUTO_KEY` | Créer clé free-tier via `POST /api/v2/auth/keys` | Non (défaut `true`) |
...
| Clé OpenSea free-tier expirée (~30 j) | Renouveler via portail developer ou laisser `OPENSEA_AUTO_KEY` recréer (limite 3/h/IP). |
...
- [ ] Clés Finnhub / CoinGecko / Zerion / OpenSea en variables serveur uniquement (pas `NEXT_PUBLIC_`). `SOLANA_RPC_URL` = URL RPC (pas de secret Solscan).
...
- `docs/opensea.md` — OpenSea API v2